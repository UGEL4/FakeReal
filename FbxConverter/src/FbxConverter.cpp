...
namespace FakeReal {
	...
	bool FbxConverter::CreateScene()
	{
		...
		else
		{
			...
			if (pRootNode)
			{
				...
				{
					m_pMeshNode = new StaticMeshNode;
	...
	void FbxConverter::GetMesh(FbxNode* pNode)
	{
		...
		for (int k = 0; (k == 0 || k < MaterialCount); k++)
		{
			...
			//创建网格
			CreateStaticMesh(UVNum, (pFBXSkin != nullptr));
		}//for k = 0
	...
	void FbxConverter::ReadVertex(FbxMesh* pMesh, int ctrlPointIndex, glm::vec3& V)
	{
		FbxVector4* ctrlPoints = pMesh->GetControlPoints();
		V.x = (float)ctrlPoints[ctrlPointIndex][0];
		V.y = (float)ctrlPoints[ctrlPointIndex][1];
		V.z = (float)ctrlPoints[ctrlPointIndex][2];
	}

	void FbxConverter::ReadNormal(FbxMesh* pMesh, int ctrlPointIndex, int vertexCounter, glm::vec3& N)
	{
		...
		case FbxGeometryElement::eByControlPoint:
		{
			...
			{
				N.x = (float)leNormal->GetDirectArray().GetAt(ctrlPointIndex)[0];
				N.y = (float)leNormal->GetDirectArray().GetAt(ctrlPointIndex)[1];
				N.z = (float)leNormal->GetDirectArray().GetAt(ctrlPointIndex)[2];
			}
			break;
			case FbxGeometryElement::eIndexToDirect:
			{
				int id = leNormal->GetIndexArray().GetAt(ctrlPointIndex);
				N.x = (float)leNormal->GetDirectArray().GetAt(id)[0];
				N.y = (float)leNormal->GetDirectArray().GetAt(id)[1];
				N.z = (float)leNormal->GetDirectArray().GetAt(id)[2];
			}
			break;
			default:
				break;
		...
		case FbxGeometryElement::eByPolygonVertex:
		{
			...
			case FbxGeometryElement::eDirect:
			{
				N.x = (float)leNormal->GetDirectArray().GetAt(vertexCounter)[0];
				N.y = (float)leNormal->GetDirectArray().GetAt(vertexCounter)[1];
				N.z = (float)leNormal->GetDirectArray().GetAt(vertexCounter)[2];
			}
			break;
			case FbxGeometryElement::eIndexToDirect:
			{
	...
	void FbxConverter::ReadUV(FbxMesh* pMesh, int ctrlPointIndex, int triangleIndex, int triangleVertexIndex, int uvIndex, glm::vec2& UV)
	{
		...
		case FbxGeometryElement::eByControlPoint:
		{
			...
			{
			case FbxGeometryElement::eDirect:
				UV.x = (float)pUV->GetDirectArray().GetAt(ctrlPointIndex)[0];
				UV.y = (float)pUV->GetDirectArray().GetAt(ctrlPointIndex)[1];
				//FBXSDK_printf("eByControlPoint UV eDirect u=%f, v=%f", vertex.UVs[0], vertex.UVs[1]);
				break;
			case FbxGeometryElement::eIndexToDirect:
			{
				int id = pUV->GetIndexArray().GetAt(ctrlPointIndex);
				UV.x = (float)pUV->GetDirectArray().GetAt(id)[0];
				UV.y = (float)pUV->GetDirectArray().GetAt(id)[1];
		...
		case FbxGeometryElement::eByPolygonVertex:
		{
			...
			case FbxGeometryElement::eIndexToDirect:
			{
				UV.x = (float)pUV->GetDirectArray().GetAt(textureUVIndex)[0];
				UV.y = (float)pUV->GetDirectArray().GetAt(textureUVIndex)[1];
	...
	void FbxConverter::CreateStaticMesh(int UVNum, bool hasSkin)
	{
		if (mVertexArray.size() == 0)
			return;

		//顶点
		DataBuffer* pVertexData = new DataBuffer;
		pVertexData->SetData(&mVertexArray[0][0], (unsigned int)mVertexArray.size(), ShaderDataType::Float3);

		//法线
		DataBuffer* pNormalData = new DataBuffer;
		pNormalData->SetData(&mNormalArray[0][0], (unsigned int)mNormalArray.size(), ShaderDataType::Float3);

		//UV
		DataBuffer* pTexCoordData[2];
		for (int i = 0; i < UVNum && i < 2; i++)
		{
			pTexCoordData[i] = new DataBuffer;
			pTexCoordData[i]->SetData(&mTexCoordArray[i][0][0], (unsigned int)mTexCoordArray[i].size(), ShaderDataType::Float2);
		}
		...
		//索引
		DataBuffer* pIndexData = new DataBuffer;
		pIndexData->SetData(&mIndexArray[0], (unsigned int)mIndexArray.size(), ShaderDataType::UInt);
